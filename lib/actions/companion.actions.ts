...
// Public versions for static generation (no authentication required)
export const getPublicCompanions = async ({ limit = 10, page = 1, subject, topic }: GetAllCompanions) => {
...
export const getPublicRecentSessions = async (limit = 10) => {
    const supabase = createPublicSupabaseClient();
    const { data, error } = await supabase.from('session_history').select(`companions:companion_id (*)`).order('created_at', { ascending: false }).limit(limit);
